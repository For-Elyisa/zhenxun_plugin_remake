...
from utils.message_builder import text, image
...
async def _(bot: Bot, event: Event, state: T_State):
    ...
    for count in range(4):
        ...
        image_file = f"/{IMG_PATH}{card_key}.jpg"

        # 特殊规则：愚者有两张
        if card_key == "愚者":
            rand = randint(1, 2)
            image_file = f"/{IMG_PATH}{card_key}{rand}.jpg"
        ...
        if isinstance(event, PrivateMessageEvent):
            ...
            logger.info(msgstr)
            msg = MessageSegment.text("".join(msgstr))
            if count < 3:
                await bot.send_private_msg(user_id=event.user_id, message=msg + image(Path(image_file)))
            else:
                await bot.send_private_msg(user_id=event.user_id, message=msg + image(Path(image_file)))
        else:
            msg = []
            ...
            if count < 4:
                ...
                if not chain:
                    logger.error(f"第{count}轮，chain_reply 返回为空")
                else:
                    logger.info(f"第{count}轮，chain_reply 返回有效消息")
                # sv.logger.info(chain)
    if isinstance(event, GroupMessageEvent):
        if not chain:
            logger.error("消息链为空，无法发送")
        else:
            await bot.send_group_forward_msg(group_id=event.group_id, messages=chain)


async def chain_reply(bot, chain, msg, image_file):
    # msg = "".join(msg)
    data = {
        "type": "node",
        "data": {
            "name": f"{NICKNAME}",
            "uin": f"{bot.self_id}",
            "content": msg + image(Path(image_file)),
            # "content": [
